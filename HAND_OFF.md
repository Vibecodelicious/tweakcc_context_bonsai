...
- **Stage 1 — DONE (parent `b47887a`, 2026-07-07).** `docs/agent-specs/hermes-agent-context-bonsai-bindings.md` — evidence layer. Frozen identity: tag `v2026.7.1`, SHA `7c1a0295…`. Rows 1/2/3/4/6 Verified, row 5 (transcript rewrite) **Partial**: core `hermes_state.archive_and_compact` does authoritative non-destructive replacement, `llm_request` middleware can replace the outgoing provider payload per-request, but no native plugin rewrite hook — a plugin would compose in-process core APIs. Gate: repository-inspecting reviewer PASS (12+ citations exact), judged accepted.
- **Next link: Stage 2 — integration-posture selection.** Consumes the Stage 1 matrix + the contract's Change-Minimization/Code Placement rules. The decision hinges on row 5's Partial: likely hybrid (plugin/middleware for guidance+gauge; prune/retrieve via in-process composition of core `hermes_state` APIs or a narrow seam). The doc's Unverified Or Weak Areas flags the open question Stage 2 must resolve first: whether the in-memory `conversation_history` baseline can be refreshed from the store mid-session by plugin-reachable means (see `agent/conversation_compression.py:371-391`). Then Stage 3 (spec pair), 4, 5, 6 per the pipeline.
