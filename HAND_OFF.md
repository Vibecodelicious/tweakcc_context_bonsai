...
## Immediate next action — bindings re-verification pass (Track A)

**Direction-doc iteration 5 is DONE** (2026-07-06, this unit, Track A; parent `8ff245d` on `spec/update-cadence-rule`). The parent's `docs/meta-loop-direction.md` is superseded in place: Current State absorbs the codex/cline/kilo execution evidence, §1.19/§1.20, the detector tag-suffix extension, and the six-of-seven Part 4 partition; a new stale-evidence bullet names the measured rot (four corrected claims in the split review; three real drifts caught by kilo's rehearsal). Reviewed per the writing guidance: Sonnet source-truth reviewer clean (all 33 cited SHAs, Part 4 numbering, §1.19/§1.20 content, test counts 16/15/14, ledger dates, End Goal diff empty); reader-state findings fixed except one pre-existing unglossed term inside End Goal ("Fable-class"), deferred because End Goal edits need owner approval — flag it at the next owner-approved End Goal revision. Gemini credentials re-checked at unit start: still absent (no `GEMINI_API_KEY`, no oauth creds in `~/.gemini`).

**Next unit — the direction doc's new Next Step: the bindings re-verification pass (owner-tier specification; cheap-tier execution if the spec allows)**. All six bound harnesses are §1.20-rate-limited until 2026-07-12/13, so no cycle may start this week; this is the one executable, cadence-independent step. Scope per the direction doc: specify and execute a re-verification pass over the six `<harness>-context-bonsai-bindings.md` documents — probe each Binding Sites table row against live source at the pinned port commit, demote failures to unverified (never silently rewrite), commit the pass as evidence cycle-plan generation can consult. Decide during specification which parts run on the cheap tier vs owner tier, and record that boundary. Acceptance (from the doc): every row of all six documents gets a verified/demoted disposition with evidence; demotions produce recorded follow-ups. When the cadence gate opens (2026-07-12/13), kilo's first routine cycle vs v7.4.1 is the natural next cycle (its acceptance plan already absorbed the v7.4.1 drift); the parked defect candidates fold into their harnesses' cycles. If Basil provisions gemini credentials, gemini-cli Stage 5 resumes immediately (below) and outranks everything.
