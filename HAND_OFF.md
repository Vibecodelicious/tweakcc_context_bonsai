...
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4, 0.73.1 cycle sealed+landed), Codex (§4.5, bound 2026-07-06 by pipeline Stage 6 — awaits its first executed cycle; E2E-04/-06 are its open live scenarios). **Unbound (§4.6)**: cline, gemini-cli (Stage 5 credentials-BLOCKED), kilo.
...
## Immediate next action — Stage 5 for cline (Track A)

**Stage 6 for codex is DONE** (2026-07-06, this unit — codex derivation tail closed). Parent commits `603bf5e` (spec: §4.5 slot table, `docs/codex-e2e-runbook.md`, §1.19 scratch-lifecycle, §1.11 pressure-test bindings, §4.6 renumber) and `3113c6a` (acceptance: pressure-test cycle plan + 14-row replay set + rehearsal evidence, §1.15 loop closed at iteration 1 — rehearsal GREEN: 14/14 cherry-picks clean onto frozen base `1fda843fbc`, cargo build green, 38 context_bonsai tests passed). Detector: 14/14 tests, live run emits `pending-target: Codex ported=0.0.2504301132 upstream=0.142.5`; port tag `bonsai/v1-on-codex-0.0.2504301132` seeded locally on the fork. Load-bearing discoveries bound in §4.5: the 14 port commits are a contiguous suffix on upstream `main` (not interleaved); the `codex/` checkout is shallow (merge-base trap); the fork links the side crate by relative path (worktree builds need the `.agent_tmp/codex_context_bonsai` symlink shim — rehearsal-proven); side crate fails `cargo fmt --check` at its own HEAD (r04 informational). Both reviewer passes + both §1.15 passes ran; all findings fixed pre-commit.

**Next unit — Stage 5 for cline** (next unbound harness; gemini-cli stays credentials-BLOCKED, kilo after cline): run derivation-pipeline-spec §8 against the cline port — bind installation+runtime e2e docs (all eight e2e-spec slots), execute Protocol A once against a real build, reviewer+judge gate. Start from `docs/agent-specs/cline-context-bonsai-spec.md` + `cline-context-bonsai-bindings.md` and the codex/pi Stage 5 precedents. Also open (either order): codex's first routine cycle when a target is chosen — its live gate must execute E2E-04/E2E-06.
