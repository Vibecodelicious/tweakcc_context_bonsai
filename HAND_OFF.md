...
## Immediate next action — cline fresh-machine install run (Track A)

**The weekly update-cadence rule is BOUND and LANDED** (2026-07-06, this unit; owner directive showrunner-relayed 2026-07-06). Parent commit `9c9ff68` on branch **`spec/update-cadence-rule`** (contains all of `pin-advance/codex-0.125.0`): forward-port spec **§1.20** — routine cycle STARTS rate-limited to one per 7 days per harness (`cadence-period-days: 7`, machine-parsed); detection stays continuous; committed ledger `docs/agent-specs/cycle-cadence-ledger.json` (seeded from each harness's latest cycle's verified commit dates; invoker updates it as its own commit at target supply, before executor launch; new-harness binding seeds its entry); owner-declared announcement-record override at `docs/announcements/<date>-<slug>.md` (three machine-checkable lines; scope `all` or named harnesses; one out-of-band start per in-scope harness per announcement; never executor-inferred — gate checks shape/timing, owner provenance is a review-enforced integrity rule). Gate: `scripts/check-cycle-cadence.mjs --harness <name> [--announcement <path>]`, exit 0/1/2/3 = eligible/usage/fail-closed/rate-limited; 15 new tests, full suite 45/45. Adversarially reviewed before landing (two independent repository-inspecting reviewers; all substantive findings fixed). A rate-limited refusal is pre-cycle — no §1.17 STOP, no reason code. **All harnesses are rate-limited until 2026-07-12/13** absent an owner announcement.

**Next unit — cline fresh-machine install-gate run**: the cline §4.6 parity claim is still open pending a fresh-machine (or clean-container) installation e2e run — the v2.17.0-cli cycle's pre-publish gate ran bundle-sourced on this rig. Procedure: `docs/cline-e2e-runbook.md` install-gate section + `cline_context_bonsai/docs/e2e-testing.md`; record results per the Evidence-paths slot (`install-e2e-results-<DATE>.md` convention). This is executable progress and not a routine cycle start, so the cadence rule does not gate it. Alternate open units: kilo Stage 5 (derivation, owner-tier); gemini-cli Stage 5 stays credentials-BLOCKED (below). Next codex/cline/pi/CC routine cycles are now subject to the §1.20 gate.
