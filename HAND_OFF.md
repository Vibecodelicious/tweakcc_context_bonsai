...
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4, 0.73.1 cycle sealed+landed), Codex (§4.5, awaits first executed cycle; E2E-04/-06 open live), Cline (§4.6, bound 2026-07-06 by pipeline Stage 6 — awaits first executed cycle; E2E-04 + fresh-machine install open). **Unbound (§4.7)**: gemini-cli (Stage 5 credentials-BLOCKED), kilo.
...
## Immediate next action — execute the Cline v2.17.0-cli cycle (Track A)

**Stage 6 for cline is DONE** (2026-07-06, this unit — judge APPROVED; the cline derivation tail is closed). Parent commits `203bbb4` (spec/detector/runbook) + `d2f00bb` (acceptance plan). What landed: forward-port-spec **§4.6 Cline slot table** (git-fork; port = 12-commit suffix on upstream `main` at `70f0e8d548`, fork links the side repo via the `file:../cline_context_bonsai` npm dep — worktree builds need the sibling symlink `cline/.agent_tmp/cline_context_bonsai` → `../../cline_context_bonsai`, currently in place from the rehearsal, deliberately retained); unbound harnesses renumbered to §4.7 (cross-refs in §1.17 and pipeline-spec §11 updated); `docs/cline-e2e-runbook.md` (grounded command bindings; three writing-guidance reviewers); detector `git-remote-tag` extended with an optional tag suffix (`git-remote-tag cline upstream v -cli` — the `-cli` release family; extension-line v3/v4 tags drop out; port tag `bonsai/v1-on-cline-2.16.0` seeded at `f97ffd26a`; live run reports pending 2.16.0→3.32.6). **§9 acceptance gate passed against a real pending release**: committed-final cycle plan `.agents/plans/story-rebase-cycle-f97ffd26a…md` targeting `v2.17.0-cli` (invoker-selected smallest pending release; 2.18.0-cli/3.32.6-cli are later cycles), 12-row replay set (checksum `0d5692204c…`), §1.15 loop closed at iteration 1 — rehearsal GREEN (12/12 clean cherry-picks, 1447 fork unit tests + side repo green, CLI reports 2.17.0), both plan reviewers' findings fixed (executable dirty-path enumeration, gate-12 hash form, Phase 6 genuine-FAIL STOP semantics).

**Next unit — execute the plan via the calibration loop** (the section below): the plan is Ready-for-Orchestration, `committed-final`, and is the meta-loop's first chance to watch a routine executor run a derivation-pipeline-emitted binding end to end. Its live e2e gate must execute the port's **first live E2E-04 (gauge)** plus the pre-publish install gate; §2.9 steps 5–6 pushes stay owner-gated (plan Non-Goals). Executor tier per the loop: `opus`/`low` with §1.18 scaffolding (intent log seeded with `RUN-START`). Real repos are the targets (this is a REAL cycle, not calibration — the worktree mechanism isolates replay; observe, don't contaminate). Also open, either order: codex's first routine cycle when a target is chosen (live gate must run E2E-04/E2E-06); kilo Stage 5; gemini-cli remains credentials-BLOCKED (below).
