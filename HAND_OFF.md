...
Track A. Run the derivation pipeline (`docs/agent-specs/derivation-pipeline-spec.md`) new-harness path, Stages 1–6, for Hermes Agent (github.com/NousResearch/hermes-agent, Python CLI, MIT), then calibration runs and a real cycle. One stage-sized unit per link.
...
- **Stage 2 — DONE (parent `f5b0268`, 2026-07-07).** Resolving Stage 1's open question surfaced a surface Stage 1 missed: Hermes's native pluggable **ContextEngine** (`agent/context_engine.py` ABC, `register_context_engine`, config `context.engine`). Core performs persisted replacement (`archive_and_compact`) and the in-memory baseline rewrite around the engine's `compress()` return; engine tools receive the live message list; the engine owns gauge state. Row 5 upgraded Partial→Verified in the bindings doc. Posture record seeded as `hermes-agent-context-bonsai-spec.md`: **plugin-only, no core seam** (Kilo/Pi branch) — side-repo plugin registering a Bonsai context engine; shape = Part 3 pure-extension variant, upstream identity = git tag (no PyPI/npm artifact). Gate: source-truth reviewer PASS, all citations verified against the frozen SHA. §5 re-checkpoint: posture re-validates at Stage 4's first integration contact.
- **Next link: Stage 3 — spec-pair generation.** Expand `hermes-agent-context-bonsai-spec.md` to the full eleven-heading contract half (headings 6/9 already seeded from Stage 2) and add the Binding Sites table + headings structure to the bindings half, per pipeline §6: contract-traceable content quoted/cited from the behavior contract (`docs/context-bonsai-agent-spec.md`), exploration content only in the bindings half, `binding: <key>` indirection between them. Gate: writing-guidance loop with a source-truth reviewer over both provenance classes. Open design points left to Stage 3 inside the stance: same-turn vs next-boundary realization of model-initiated prune; guidance-channel split (tool descriptions vs `pre_llm_call` append); `compression.in_place` requirement wording. Then Stages 4, 5 (credentials wake below), 6.
- **Scratch clone**: `/tmp/hermes-bonsai-derivation/hermes-agent` at the frozen tag — preserved live derivation state (§1.19: delete after Stage 6 seals or on STOP disposition).
- **Credentials wake condition (owner, verbatim intent, 2026-07-07)**: "You will run into a point where I need to get OpenAI and Zen credentials setup on the agent. Slack me when you are there." When the derivation first needs a live model inside Hermes (Stage 4 real-entry-point evidence or Stage 5 e2e drives), slack Basil stating exactly which provider(s) and which Hermes config file/env/command are needed, park only that step, continue parallel non-live work. Do NOT wire credentials into Hermes yourself.
