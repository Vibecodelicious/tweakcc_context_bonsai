...
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4, 0.73.1 cycle sealed+landed), Codex (§4.5, awaits first executed cycle; E2E-04/-06 open live), Cline (§4.6, **v2.17.0-cli cycle sealed 2026-07-06** incl. first live E2E-04 PASS; fresh-machine install still open; 2.18.0-cli/3.32.6-cli pending as later cycles). **Unbound (§4.7)**: gemini-cli (Stage 5 credentials-BLOCKED), kilo.
...
- **Tiering evidence**: four harnesses, two tiers — Opus-4.8-low executes validated plans clean (CC, pi, and cline cycles; the cline run was the first end-to-end consumption of a derivation-pipeline-emitted binding, sealed with zero EXECUTOR-FAIL); gpt-5.3-codex-spark completes with §1.18 scaffolding (run 6 clean).
...
## Immediate next action — codex's first routine cycle (Track A)

**The Cline v2.17.0-cli cycle is EXECUTED and SEALED** (2026-07-06, this unit): a single Workflow executor at `opus`/`low` ran plan Phases 0–8 + §1.16 end to end in ~33 minutes, **zero EXECUTOR-FAIL stumbles**, all seal gates green — the meta-loop's first proof that a derivation-pipeline-emitted binding carries a routine executor unaided. Durable state, all local and verified against git: cline fork — 12 replayed commits on `07593bb42a` (`v2.17.0-cli`), branch `replay/context-bonsai-on-cline-2.17.0` + tag `bonsai/v1-on-cline-2.17.0` at `fc6c46fd54`, submodule checkout detached there; side repo — e2e results commit `1ce7cbc7` (`docs/e2e-results-2026-07-06-cline-2.17.0-live.md` + Run records row); parent — working line is now branch **`pin-advance/cline-2.17.0`** (contains all of the old line): `cfc03e1` pin advance + validation artifacts, `30ef528` owner-tier fold of the run's three SPEC-GAPs into §4.6 **and** `docs/cline-e2e-runbook.md` (E2E-04 must be driven in a single ≥5-API-request process — `apiRequestCount` resets per process; pre-publish install gate runs on real disk, not /tmp tmpfs; same-date results docs take a `-cline-<version>-` suffix), reviewer-verified with its one blocking finding (stale "never executed live" wording) fixed pre-commit. First live E2E-04 PASSED; full parity claim still waits on a fresh-machine install run. §2.9 steps 5–6 pushes remain owner-gated; run scratch removed per §1.19.

**Next unit — codex's first routine cycle**: choose a target via `scripts/detect-pending-target.mjs --harness Codex` (detection, not selection — the invoker picks, smallest-pending precedent from the cline cycle), generate the cycle plan through the §1.15 loop against §4.5, then execute via the calibration loop below; its live gate must run codex's open E2E-04/E2E-06. Also open: kilo Stage 5; a cline fresh-machine install run (closes the parity claim); gemini-cli remains credentials-BLOCKED (below).
