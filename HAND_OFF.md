...
**THE TIERING DECISION IS MADE: branch 1** (Basil, 2026-07-03, direct instruction to the relay: "Branch 1 — up-tier plan generation, amend the End Goal accordingly"). Recorded with the End Goal amendment at `b4f02cf`; run-4 residue and records archived to `.agents/pilot/archive/run4-v1.17.13/` and the residue cleared from the tree (§1.14 clean). Execution owed per the decision record, in order: (1) **generate and validate the v1.17.13 cycle plan on the Fable-class tier** per `docs/agent-specs/forward-port-spec.md` — same spec, same §1.15 validation loop and reviewer gates, only the generating tier changes; run-4's archived plan shows what the reviewers demanded and where GPT-5.5 fell short (verbatim runbook transcription, concrete pin-bump commands, unambiguous install-gate result ordering); (2) hand the approved plan to the GPT-5.5 orchestrator for execution + §1.16 maintenance under the run-4 observation protocol (`launch-run.sh`, exit marker teed into the run log, ≤30-minute sweeps, 429 self-resume expectation); (3) the runbook's COMPOSED command sequences get their first live run at that cycle's gates — a composed-command failure is a finding against the runbook, not license to improvise.

Worktree state, deliberate: parent tree is clean except the `tweakcc_context_bonsai` submodule pointer (untouchable pin — never commit it). Window `bonsai:@4` (the finished pilot run) is inert; `@7`, `@1`, `@5` remain inert and Basil's to close.
...
**Begin execution-owed item (1): generate the v1.17.13 cycle plan on the Fable-class tier.** Re-read `docs/agent-specs/forward-port-spec.md` in full before generating (Basil's re-read rule extends to it; do not work from this summary), plus `.llm-conductor/planning_guidance.md` if the spec's plan-generation sections invoke it. Inputs: the frozen v1.17.13 cycle facts are unchanged from the pilot runs (`SOURCE_HEAD_SHA 0dfbeeda…​`); the archived run-4 plan (`.agents/pilot/archive/run4-v1.17.13/story-rebase-cycle-…​.md`, iteration log at its tail) is reference material for what the §1.15 reviewers require — consult it for the demanded concreteness, but generate fresh, don't patch it. The plan must pass the same §1.15 reviewer validation loop; copy runbook commands verbatim (the run-4 lesson), bind the pin-bump commands and install-gate result ordering concretely, and flag any genuinely missing binding rather than inventing (§4.2). This is judgment-tier work: the generating model is you, but delegate bulk source reading to cheap subagents where sensible (credit economy). Plan generation may span multiple relay sessions — update this file at each hand-off. Once a plan passes validation, item (2) is the GPT-5.5 execution launch under the run-4 observation protocol.
