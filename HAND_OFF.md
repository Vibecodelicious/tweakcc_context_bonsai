...
- **Meta-loop docs**: forward-port spec + 20-code §1.17 escalation registry + §1.18 run-continuity duties; `derivation-pipeline-spec.md` (6 stages, owner-tier); dispatcher `scripts/dispatch-escalation.mjs` and release detector `scripts/detect-pending-target.mjs` (both parse the spec at runtime, fail closed; 14/14 + 16/16 tests — detector's `git-remote-tag` now takes an optional literal tag suffix for Cline's `v<version>-cli` family).
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4, 0.73.1 cycle sealed+landed), Codex (§4.5, **rust-v0.125.0 cycle sealed 2026-07-06** incl. first live E2E-04 and E2E-06 PASS; 0.126.0…0.142.5 pending as later cycles), Cline (§4.6, v2.17.0-cli cycle sealed 2026-07-06; fresh-machine install still open; 2.18.0-cli/3.32.6-cli pending). **Unbound (§4.7)**: gemini-cli (Stage 5 credentials-BLOCKED), kilo.
...
## Immediate next action — weekly update-cadence rule (Track A, owner directive 2026-07-06)

**The codex rust-v0.125.0 cycle is EXECUTED and SEALED** (2026-07-06, this unit — codex's first routine cycle): a single Workflow executor at `opus`/`low` ran plan Phases 0–8 + §1.16 in ~57 minutes; all seal gates green; §2.9 steps 1–4 complete, steps 5–6 pushes owner-gated. Durable state, verified against git: codex fork — 14 replayed commits on `637f7dd6d7` (`rust-v0.125.0`), branch `replay/context-bonsai-on-codex-0.125.0` + tag `bonsai/v1-on-codex-0.125.0` at `79bf2a6e64`, submodule detached there, source branch untouched; side crate — e2e results commit `e15634c` (`docs/e2e-results-2026-07-06-live.md`); parent — branch **`pin-advance/codex-0.125.0`** (contains all of `pin-advance/cline-2.17.0`): `4c7328e` spec §1.14 same-source different-target `-on-<target>` keying, `dde09e4` the §1.15-validated cycle plan (target selected as smallest stable tag **containing the port base** — version-sorted smallest-pending is a year of stale datever history behind the base; plan binds the two known cherry-pick conflicts), `e9a3af4` pin advance + validation artifacts, `725a7ae` owner-tier fold of the run's three SPEC-GAPs (§4.5 r01 baseline lockfile churn → restore before replay; runbook install gate drops `--depth 1` — non-tip pin unfetchable shallow; runbook E2E-04 no-gauge control only clean pre-first-cadence). Live e2e all PASS including the port's first live E2E-04 (gauge) and E2E-06 (resume persistence); Protocol A passed after the executor self-corrected two oracle-drive contaminations (EXECUTOR-FAIL, self-recovered). Install gate PASS via the runbook's new `file://`-sourced block (fork is shallow; bundles from it are unclonable — verified). Run scratch removed per §1.19. Detector now reports codex ported=0.125.0.

**Next unit — weekly update-cadence rule (owner directive, showrunner-relayed 2026-07-06, verbatim intent)**: "We should setup a rule to update each client only once per week unless there is a major announcement from a vendor. (e.g. if there is a new GPT model, all of them should be updated. If there is a new claude cli feature, just the claude one should be updated)." Bind at spec level as a reviewed unit: (a) routine cycle STARTS rate-limited to once per week per harness against a machine-checkable last-cycle-start record (the detection/dispatch layer — likely `scripts/detect-pending-target.mjs` or a sibling gate + a committed timestamp record; detection itself stays continuous); (b) major-announcement override, owner-declared or explicitly evidenced — never executor-inferred — with scope following the announcement (vendor-wide capability → all harnesses eligible immediately; harness-specific feature → that harness only). Run through adversarial review before landing. Also open: kilo Stage 5; cline fresh-machine install run (closes its parity claim); next codex/cline/pi/CC targets now subject to the cadence rule once it lands.
