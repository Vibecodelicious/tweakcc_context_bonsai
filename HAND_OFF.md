...
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4 pure-extension variant, 0.73.1 cycle sealed+landed — first non-CC harness through the whole pipeline on the weak tier). **Unbound (§4.5)**: cline, codex, gemini-cli, kilo.
...
## Immediate next action — UNIT IN PROGRESS (Track A)

Executing the direction doc's Next Step, three steps:

1. **Pipeline-spec fold DONE** (this unit, 2026-07-06): parent `b49d69a` folds the pi run's three lessons into the derivation-pipeline stage output contracts — Stage 2 records the Part 3 variant (closed patched bundle vs pure extension); Stage 5 binds the evidence-channel rule (a scenario binding must name a channel that provably carries the asserted evidence — the pi E2E-01 `--mode json` defect — with automated/manual splits explicit); Stage 6 binds the EXECUTED/SOURCE-VERIFIED/COMPOSED runbook grounding taxonomy. Independent Opus reviewer pre-commit: APPROVE, zero blocking; one non-blocking gloss removed. Local-only.
2. **Stage 5 target PIVOTED gemini-cli → codex (2026-07-06, DRIVE-rule decision).** Gemini-cli was the scouted first pick and its bring-up succeeded — side repo rebuilt, fork `npm install` re-bundled (the pre-existing bundle was stale, missing the bonsai wiring; now grep-verified present in `bundle/chunk-*.js`), CLI smoke-tested at `0.41.0-nightly` — but the live run is **credentials-BLOCKED**: `~/.gemini` auth is `oauth-personal` with `google_accounts.json` `active: null` (logged out, no cached token — the CLI hangs waiting for an interactive browser login), no `GEMINI_API_KEY` in the environment, and the auto-mode permission classifier denies further credential exploration (correctly — same class as the run-6 park). BLOCKED is recorded, not waived; Basil FYI'd with the two provisioning routes (interactive `gemini` login from the fork build, or export `GEMINI_API_KEY`). **Codex is the pivot target**: equally unbound in Part 4, and its Protocol A live PASS is already on file at the *current* fork HEAD `5a680bcb3a` (`codex_context_bonsai/docs/e2e-results-2026-05-20-live.md` — full PASS incl. the secret-prune oracle), so Stage 5 for codex = author the bound installation+runtime e2e doc (all eight e2e-spec slots) citing that EXECUTED evidence, no new credentials needed; reviewer loop; commit in `codex_context_bonsai/`. Record in the doc why the 2026-05-20 evidence satisfies "executed once against a real build" (fork HEAD unchanged since the run). Codex facts from the scout: Rust crate compiled into the patched fork, from-source `cargo build --bin codex` known-working; only self-declared gap = gauge cadence/severity validation not attempted. When gemini credentials arrive, gemini-cli Stage 5 resumes exactly at the live drive (everything else is done; work dir `/tmp/gemini-bonsai-e2e/run-cwd`; drive order + traps in the operational brief section of this unit's session — key traps: ≥1 persisted turn before first prune or the BeforeTool hook blocks; archive lands under the project temp dir `context-bonsai/session-<id>.json`; tools are `context-bonsai-prune`/`-retrieve` over an injected MCP server).
3. **Stage 6 for codex**: emit the Part 4 slot table + runbook (grounded per the new EXECUTED/SOURCE-VERIFIED/COMPOSED taxonomy); acceptance = detector parses the new slot unmodified + a §1.15-validated routine cycle plan (pressure-test mode if no pending release — note codex upstream is consumed as a git fork, npm irrelevant per the scout).
