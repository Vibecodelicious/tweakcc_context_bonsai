...
## Immediate next action — chain signal-to-landing (Track A, Provisional Future Step 2)

**Freshness consultation is WIRED** (2026-07-06, this unit, Track A; parent `fdede6a` on `spec/update-cadence-rule`). `forward-port-spec.md` §1.15 now opens with a bindings-freshness consultation step (both shapes): read the most recent `bindings-reverification-<date>.md`; a DEMOTED Binding Sites row the plan would bind is generation-blocking and unresolvable by plan revision — immediate STOP with `generation-nonconvergence` (registry row amended; resume = Stage-1 re-exploration of the row, then regenerate); a pass record older than the harness's `last_cycle_start` in the cadence ledger flags the bindings stale (non-blocking); a missing pass record is recorded as vacuous, never invented; OpenCode exempt (Part 4-hosted bindings). `bindings-reverification-spec.md`'s two "not yet wired" statements now point at the wired step. 45/45 script tests pass; source-truth and reader-state reviews clean on iteration 1. The direction doc's Next Step (re-verification pass + freshness consultation) is now fully done.

**Next unit — chain the routine path end-to-end (direction doc Provisional Future Step 2).** Detection (`detect-pending-target.mjs`), cadence gating (`check-cycle-cadence.mjs` + ledger), freshness consultation (§1.15, just wired), generation, execution, escalation routing (`dispatch-escalation.mjs`), and local landing are each individually built or ruled routine; what remains is chaining them so a routine release consumes zero owner decisions between detector signal and sealed local landing (outward actions stay owner-gated). Scope for the unit: design the invoker chain (likely a spec section + an invoker script that runs detector → gate → ledger record → generation launch with the §1.18 seed), sized to what one session can validate; it need not execute a full live cycle — the cadence gate blocks all six harnesses until 2026-07-12/13 anyway. A brief direction-doc iteration (its Next Step section is now stale) can fold into this unit's close if cheap, or wait for the chaining evidence. When the cadence gate opens, kilo's first routine cycle vs v7.4.1 is the natural pick — ideally driven through the new chain. If Basil provisions gemini credentials, gemini-cli Stage 5 resumes immediately (below) and outranks everything.
