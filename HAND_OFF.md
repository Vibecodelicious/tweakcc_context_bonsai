...
## State as of 2026-07-03 ~05:10 UTC (run 4 in expected quota stall)

**PILOT RUN 4 IS LIVE but stalled on provider quota — the run-3 pattern, anticipated by launch prep.** Launched 03:36 UTC in tmux window `bonsai:@4` (`gpt55-pilot-run4`), orchestrator PID 1863670, run log `.agents/pilot/gpt55-v1.17.13-run.log` (tee includes the exit marker — plumbing dry-run-verified). At 03:52:15 a `bonsai-reviewer` subagent LLM call failed with `Subscription quota exceeded. Retry in 3hr 43min` (pilot's opencode log `~/.local/share/opencode/log/2026-07-03T033604.log`); the session sleeps it out in-process and **self-resume is expected ~07:35 UTC** — verify self-resume before considering any injection, and injection needs watchdog sanction. Pre-stall progress was healthy: preflight clean (§1.14 globs empty, release range enumerated), story plan drafted and iterated, reviewer agents mid-flight on final ambiguity/missing-details passes; the executor found `docs/opencode-e2e-runbook.md` via §4.2 immediately. Milestones (launch, stall) are recorded in the observer log. Three wake-and-verify sweeps done (03:38, 04:38, 05:08): process alive, log silent since 03:52 — explained by the stall.

The watchdog chose branch 1 (run 4, unchanged terms) with the owner slacked and the override window explicitly waived ("launch it, window's yours to hold"). Decision + launch prep at `da89bd8`: residue archived to `.agents/pilot/archive/run3-v1.17.13/`, 429-self-resume expectation in `launch-run.sh`, fresh observer log armed with the eight-gap recurrence checklist.
...
**Continue observing run 4 through the quota stall**: ≤30-minute wake-and-verify sweeps (process alive: `kill -0 1863670`; run-log growth: `stat -c %s .agents/pilot/gpt55-v1.17.13-run.log`, 9870 bytes at stall; exit marker: grep `PILOT-PROCESS-EXITED`). A useful sweep pattern: a background loop that exits early on marker/death/log-growth, else at 29 minutes. Expect nothing before **~07:35 UTC**; at that horizon verify self-resume (log growing again). If the horizon passes ~15+ minutes with no growth, re-check the pilot's opencode log for a fresh quota error (a second stall extends the horizon — record it and keep sweeping); anything else unexplained goes to the watchdog before any intervention. Record milestones and verdicts in the observer log as they occur; keep sessions short by relaying between sweeps when a session grows long (the credit economy), or hold a background timer within one session.

On `PILOT-PROCESS-EXITED`: post-run sequence (verdicts + acceptance assessment against the eight-gap checklist in the observer log → commit the four records mirroring `bddcf96` → owner review of any uncommitted §1.16 Part-4 edit → iterate the direction doc → slack Basil, verify `ok`). The isolated capability question this run answers is stated in the observer log's header; a seal, a clean EXECUTOR-FAIL, and an explicit early missing-binding STOP are all unconfounded outcomes — do not rescue any of them.
