...
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4, 0.73.1 cycle sealed+landed), Codex (§4.5, **rust-v0.125.0 cycle sealed 2026-07-06** incl. first live E2E-04 and E2E-06 PASS; 0.126.0…0.142.5 pending as later cycles), Cline (§4.6, v2.17.0-cli cycle sealed 2026-07-06; **fresh-machine install gate PASS 2026-07-06 — automated parity gate closed**, only the manual VS Code checkpoint half open; one defect candidate: cross-process `retrieve_same_step` false rejection, recorded for the next cycle; 2.18.0-cli/3.32.6-cli pending). **Unbound (§4.7)**: gemini-cli (Stage 5 credentials-BLOCKED), kilo.
...
## Immediate next action — kilo Stage 5 derivation (Track A)

**The cline fresh-machine install gate is DONE — PASS** (2026-07-06, this unit, Track A). Side repo `37fdf22` (results doc `cline_context_bonsai/docs/install-e2e-results-2026-07-06.md` + procedure-doc fold), parent `498a0ab` (runbook fold + gitlink). Executed as a clean `node:22` podman container (chosen over a sprite — pre-publish repo content must not leave the machine), bundle-sourced from single-ref bundles on real disk; install phases in the container, live registration + prune/retrieve smoke on the host against the container-built bundle (`claude-code` provider; the split is recorded in the results doc). All phases PASS; the cline automated parity gate is closed. The smoke surfaced a defect candidate: `retrieve_same_step` falsely rejects a retrieve issued as the first API request of a resumed process when the prune landed on the first request of its own process (guard counter = per-process `apiRequestCount`, the E2E-04 counter); executed workaround = one in-process retry; disposition = next cline cycle. Scratch removed per §1.19 (`podman unshare rm -rf` — container-created files need it).

**Next unit — kilo Stage 5 (derivation, owner-tier)**: run the derivation pipeline (`docs/agent-specs/derivation-pipeline-spec.md`) for kilo, the last unbound harness with an executable path. Gemini-cli Stage 5 stays credentials-BLOCKED (below). Routine cycles for all bound harnesses are §1.20 rate-limited until 2026-07-12/13 absent an owner announcement (cadence rule landed at parent `9c9ff68`; gate `scripts/check-cycle-cadence.mjs`, exits 0/1/2/3 = eligible/usage/fail-closed/rate-limited; ledger `docs/agent-specs/cycle-cadence-ledger.json`).
