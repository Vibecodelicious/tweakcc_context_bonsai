...
Track A. Run the derivation pipeline (`docs/agent-specs/derivation-pipeline-spec.md`) new-harness path, Stages 1–6, for Hermes Agent (github.com/NousResearch/hermes-agent, Python CLI, MIT), then calibration runs and a real cycle. One stage-sized unit per link. **Stages 1–4 DONE and epic-sealed 2026-07-07** — full record in `HAND_OFF_ARCHIVE_2026-07-07.md`; frozen identity tag `v2026.7.1`, SHA `7c1a0295…`; posture plugin-only ContextEngine, all 12 Binding Sites keys realized except `presentation-middleware` (deliberate).

- **Epic seal DONE (2026-07-07):** side repo re-homed as parent submodule `hermes_context_bonsai` (parent `21967fd`; bare at `/home/basil/projects/.context-bonsai-side-bares/hermes_context_bonsai.git`, checkout origin file://, `.gitmodules` URL = future Vibecodelicious fork per the established pattern — no fork/push done, owner-gated). Binding Sites completion folded (parent `2bf7be8`, repo-inspecting review PASS). Verified: frozen clone pristine at the SHA, side repo clean at `f5ecfbe`.
- **Cleanup residue for Basil (non-blocking):** the classifier denied `rm -rf /home/basil/scratch/hermes-bonsai-stage4/` (§1.19 removal of the now-superseded Stage 4 work root). One-liner for Basil; nothing in it is live state.
- **Next link: Stage 5 (calibration/e2e slot binding).** Pipeline spec §4 Stage 5 for Hermes: e2e slot binding + live-model Protocol A drives. The live-model drives FIRE the credentials wake condition (below) — on reaching that step, slack Basil naming the exact provider(s) and the Hermes config file/env/command needed, park only that step, and continue parallel non-live Stage 5/6 work (e2e doc authoring, slot-table prep). Operator install surface: symlink/copy `context-bonsai/` from the submodule into `$HERMES_HOME/plugins/` + `context.engine: bonsai` (side repo README has the full procedure).
- **Scratch state**: `/tmp/hermes-bonsai-derivation/hermes-agent` (frozen-tag derivation clone — §1.19: delete after Stage 6 seals); `/home/basil/scratch/hermes-bonsai-stage4/` (superseded, pending Basil's rm above).
- **Key learned facts**: zero-credential real-entry-point evidence works — a stub OpenAI-compatible server (`model.provider: custom` + local `base_url` + `model.context_length` pinned in config) drives the genuine conversation loop; the stub's recorded request payloads are the model-visible-context evidence channel. Shell-wedge symptom (every Bash exits 1, empty) = /tmp quota full; recover with `dangerouslyDisableSandbox` Bash.
- **Credentials wake condition (owner, verbatim intent, 2026-07-07)**: "You will run into a point where I need to get OpenAI and Zen credentials setup on the agent. Slack me when you are there." Stage 4 did NOT trigger it (stub provider sufficed). When Stage 5's live-model e2e drives arrive, slack Basil stating exactly which provider(s) and which Hermes config file/env/command are needed, park only that step, continue parallel non-live work. Do NOT wire credentials into Hermes yourself.
