...
## State as of 2026-07-03 ~02:45 UTC

**PILOT RUN 3 IS LIVE** in tmux window `bonsai:@7` (`gpt55-pilot-run3`), original process and session intact, launched ~23:03 UTC 2026-07-02 via `.agents/pilot/launch-run.sh` under unchanged terms (same brief, same frozen inputs, GPT-5.5 orchestrator at medium reasoning). Milestones so far, all recorded with evidence in the observer log (`.agents/pilot/gpt55-v1.17.13-observer-log.md`, untracked by design):
...
- **The §1.15 validation loop CONVERGED** — reviews, README manual approval + judge, hash-pinning, hash gate, rereviews all ✓ by 23:12:50 UTC, two iterations against the 3-cap, minutes not hours. The loop that exhausted runs 1 and 2 cleared cleanly once no known spec gaps taxed its budget. Provisional-positive on capability question (a).
- **Quota stall 23:13–02:35 UTC, self-recovered, zero contamination.** The provider returned 429 `BlackUsageLimitError` (retry-after 3h22m); the process slept it out and resumed on its own at the reset — the observer's initial "no retry scheduled" reading was wrong, and the watchdog-sanctioned resume injection was staged but **never used** (script parked in the observing session's scratchpad, deliberately outside the repo). External interruption, no verdict. Post-resume the executor picked up exactly where it stalled: plan edits, then final Missing-details + Ambiguity reviewer passes, both ✓.

The run is now heading into plan commit and replay execution — the heavy phase. Quota may exhaust again mid-run: on every silent wake, check the newest `~/.local/share/opencode/log/*.log` for `session.processor` errors before concluding anything, and know that a quota-stalled process looks exactly like a working one from the run log alone (alive, low CPU, log frozen, no exit marker). Basil's override stands: killing `bonsai:@7` aborts the run.
...
**Observe run 3 to its end; record verdicts; do not interfere** (intent rules 4 and 6 bind in full):

1. Arm a background watcher on `.agents/pilot/gpt55-v1.17.13-run.log` for `PILOT-PROCESS-EXITED` with a **≤30-minute timeout** (`timeout 1800 grep -q -m1 ... <(tail -f ...)`). On every firing — match or timeout — verify actual state before re-arming: log growing? Process alive and ticking (`ps --ppid` under the `bonsai:@7` pane PID; CPU-tick delta over a few seconds)? **A silent log plus a live process is not evidence of progress** — a quota-stalled process looks exactly like a working one from the run log alone, so on every silent wake also check the newest `~/.local/share/opencode/log/*.log` for `session.processor` errors. A quota stall sleeps out its retry-after in-process and self-resumes; if one appears, note it in the observer log (external, no verdict) and keep waking — do not inject anything.
2. As stumbles appear, append verdicts (`SPEC-GAP` vs `EXECUTOR-FAIL`, with evidence) to the observer log. Recurrence of ANY of the six fixed SPEC-GAPs (listed in the observer log header) is owner-tier rework, not pilot data.
3. The §1.15 convergence is already observed and provisional-positive on capability question (a); what remains to answer the acceptance test is the plan commit, replay execution through reviewer/judge/e2e gates (multi-hour developer passes are normal here), and §1.16 maintenance in whichever outcome.
4. At run end: verify final report + §1.16 maintenance outcome exist per the brief; complete the observer log's assessment against its acceptance test; iterate the direction doc; slack Basil (verify `ok`); relay or stop per the relay rules. Expect the run to span several relay units — split at natural seams and keep this file current, committing HAND_OFF updates in this side repo as prior units did.
