...
## Hermes Agent derivation — COMPLETE (2026-07-07; owner directive "add context bonsai to Hermes agent" fully discharged)

Track A. The pipeline's first full new-harness run: Stages 1–6 plus the Stage 6 acceptance-gate calibration are all sealed. Full records: Stages 1–5 + epic seal + Stage 6 emission + calibration runs in `HAND_OFF_ARCHIVE_2026-07-07.md`. Closing facts:

- **Calibration (this unit)**: run 1 STOPped at Phase 6 (live drive outran the 2-min default tool timeout; executor backgrounded it and ended its turn — SPEC-GAP + EXECUTOR-FAIL, both dispositioned); Phase 6 wall-time binding added (reviewed, parent `0e3e9af`); **run 2 SEALED end-to-end with zero STOPs and zero stumbles** — 91/91 pytest, ruff clean, 8/8 matrix, live Protocol A `DRIVE PASS` on 5 host-state verdicts, final verification green. Maintenance report + baseline artifact committed: parent `372560f`. Executor: Opus 4.8 low (tiering evidence extended — first-run-clean on the amended plan).
- **Scratch state**: all `/tmp/hermes-bonsai-*` state removed (plan Phase 9 + invoker sweep, §1.19). Only `/home/basil/scratch/hermes-bonsai-stage4/` remains — superseded, pending Basil's own `rm` (classifier denies it for us).
- **Credentials**: Stage 5/6 + calibration all ran zero-credential; the owner's "slack me when you need OpenAI/Zen credentials" condition never fired for required work. Wiring notes for optional real-provider drives were slacked 2026-07-07. Do NOT wire credentials into Hermes ourselves.
- Next Hermes act: a §1.20 routine cycle when upstream releases past v2026.7.1 (cadence-rate-limited until 2026-07-14; detector slot live, `up-to-date`).

## STEADY STATE backlog (ACTIVE again — the Hermes thread above is closed; parked 2026-07-06, wake conditions below stand)
