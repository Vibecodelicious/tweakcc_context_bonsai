...
## Immediate next action — wire freshness consultation into generation (Track A)

**Bindings re-verification pass is DONE** (2026-07-06, this unit, Track A; parent `e8f0b68` on `spec/update-cadence-rule`). Two new docs in `docs/agent-specs/`: `bindings-reverification-spec.md` (the repeatable procedure — cheap-tier read-only probes per Binding Sites row, owner-tier dispositions, demote-don't-rewrite, committed pass record) and `bindings-reverification-2026-07-06.md` (first pass: six Sonnet probes at the pinned submodule SHAs; **36 rows, 34 VERIFIED with file:line evidence, 2 DEMOTED** — claude-code `gauge-channel` (no gauge logic exists in current source) and pi `searchable-text` (tool field names wrong vs the pinned pi-ai schema); both annotated in place, follow-ups route to those harnesses' next-cycle Stage-1 re-exploration; codex line-anchor drift noted as a non-demotion observation). Spec reviewed per writing guidance: source-truth clean, two reader-state findings fixed. Gemini credentials re-checked at unit start: still absent.

**Next unit — wire pass-record consultation into routine cycle-plan generation.** The new spec records this as unwired future work, and the direction doc's Provisional Future Step 2 (close the remaining human steps in the routine path) names freshness consultation as an input to generation. Scope: add a generation-time step to `forward-port-spec.md`'s routine cycle-plan generation — read the most recent `bindings-reverification-*.md` pass record; a DEMOTED row the plan would bind is a generation-blocking finding (Stage-1 re-exploration first); a bindings doc whose latest pass predates the harness's last sealed cycle is flagged stale. Small owner-tier spec edit + writing-guidance review; keep it minimal. When the §1.20 cadence gate opens (2026-07-12/13), kilo's first routine cycle vs v7.4.1 is the natural next cycle (its acceptance plan already absorbed the v7.4.1 drift) — and it must consume the two demotion follow-ups relevant to its harness (none) plus the freshness check. If Basil provisions gemini credentials, gemini-cli Stage 5 resumes immediately (below) and outranks everything.
