...
2. **Stage 5 for gemini-cli — IN PROGRESS.** Harness choice re-scouted 2026-07-06 (four-harness scout): **gemini-cli** — cheapest first-time bring-up (JS fork already built locally, existing local Google auth `~/.gemini/` + ADC); codex excluded because its Protocol A live PASS is already sealed at current fork HEAD `5a680bcb3a` (`codex_context_bonsai/docs/e2e-results-2026-05-20-live.md`) so it adds no new Stage-5 coverage; cline (~1.4k commits adrift, VS Code fork build) and kilo (~6.9k adrift, nonstandard loader) are gated behind it. Deliverable: bound installation+runtime e2e doc (all eight e2e-spec slots) + one live Protocol A PASS against the real fork build, reviewer loop, committed in `gemini-cli_context_bonsai/`. Known trap: fork pinned 0.41.0-nightly vs npm 0.49.0 (8 minors stale) — model-name availability risk; fork-based run is still the correct Stage-5 target.
3. **Stage 6 for gemini-cli**: emit the Part 4 slot table + runbook (grounded per the new taxonomy); acceptance = detector parses the new slot unmodified + a §1.15-validated routine cycle plan (pressure-test mode if no pending release).
