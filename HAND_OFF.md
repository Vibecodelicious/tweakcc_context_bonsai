...
- **Part 4 bound harnesses**: OpenCode (1.17.13), Claude Code (§4.3, 2.1.201 cycle sealed+landed), Pi (§4.4 pure-extension variant, 0.73.1 cycle sealed+landed — first non-CC harness through the whole pipeline on the weak tier). **Unbound (§4.5)**: cline, codex (Stage 5 done, Stage 6 pending), gemini-cli (Stage 5 credentials-BLOCKED), kilo.
...
## Immediate next action — Stage 6 for codex (Track A)

**Stage 5 for codex is DONE** (2026-07-06, this unit): `codex_context_bonsai/docs/e2e-testing.md`, committed `5235cd3` in the side repo. Combined installation+runtime binding, all eight e2e-spec slots, evidence-channel rule applied per the folded pipeline spec. E2E-01/-02/-03/-07 cite the 2026-05-20 live run as EXECUTED (valid: codex fork HEAD `5a680bcb3a` unchanged and verified clean; side-crate commits since the run are docs-only, so the built artifact's code identity is unchanged — the argument is recorded in the doc). E2E-04 (gauge) and E2E-06 (resume) are BOUND, NOT YET EXECUTED and block a full parity claim; E2E-05 is JUSTIFIED NON-LIVE (compiled-in seam not removable from a real build). Reviewer loop ran (source-truth + reader-state/task-success, both sonnet): one blocking source-truth fix (on-disk archive JSON carries `"bonsai_archive"`, never the Rust type name) plus reader fixes (multi-turn drive = `codex exec --json` then `codex exec --json resume <session-id>`, dangling references, status-label vocabulary) — all applied before commit.

**Next unit — Stage 6 for codex**: emit the Part 4 slot table + runbook (grounded per the EXECUTED/SOURCE-VERIFIED/COMPOSED taxonomy); acceptance = detector parses the new slot unmodified + a §1.15-validated routine cycle plan (pressure-test mode if no pending release — codex upstream is consumed as a git fork, npm irrelevant). Note for the runbook: Stage 5's E2E-04/E2E-06 remain unexecuted — the slot's e2e procedure should carry them as the open live scenarios.
...
**Environment warning — /tmp user quota EXCEEDED** (hit 2026-07-06 during this unit): the /tmp tmpfs user quota (6265M, 361k files) is full; writes to /tmp intermittently fail (`Disk quota exceeded`), which degrades shell output and anything staging temp files. Main consumers are pre-existing e2e/calibration dirs (`/tmp/opencode-install-e2e-w8xgqD` 1.2G, `/tmp/cc-bonsai-artifacts` 665M, `/tmp/bundles` 292M, seven `pi-cal-*`/`cc-cal-*` scratch clones ~200M each). The auto-mode classifier denies this session deleting them (pre-existing shared dirs). The sealed-cycle `pi-cal-*`/`cc-cal-*` scratch clones (~1.4G) are safe to delete — cycles sealed, clones re-provisionable — if Basil clears it or deletes them himself; do NOT touch run-6 opencode state or `/tmp/gemini-bonsai-e2e`. Repo/disk writes are unaffected (114G free). Slacked 2026-07-06.
