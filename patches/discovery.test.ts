...
const defaultTargetBundlePath = '.artifacts/claude-code/2.1.200/linux-x64/extracted.js';
const targetManifestPath = '.artifacts/claude-code/2.1.200/linux-x64/manifest.json';
...
describe('runtime helper discovery', () => {
  ...
  test('resolves the optional current-session helper shape from native 2.1.156', () => {
    const content = `
...
test('target artifact visibility-predicate disambiguation selects one switch candidate when available', async () => {
  const extractPath = process.env[targetBundleEnv] ?? defaultTargetBundlePath;
  if (!existsSync(extractPath)) {
    console.warn(
      'SKIP Claude Code native 2.1.200 Linux x64 target discovery: ' +
