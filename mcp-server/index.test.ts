...
describe("patch-presence guard", () => {
  ...
  test("prune marker includes interleaved system metadata rows", async () => {
    const sessionId = `test-session-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const sessionPath = await createSessionWithMessages(sessionId, [
      {
        type: "user",
      ...
      },
      {
        type: "user",
        uuid: "msg-1",
        message: { role: "user", content: "start mixed range" },
        timestamp: new Date().toISOString(),
        parentUuid: "initial-msg",
        sessionId,
      },
      {
        type: "system",
        uuid: "system-local-command",
        subtype: "local_command",
        message: { role: "system", content: "pwd" },
        timestamp: new Date().toISOString(),
        parentUuid: "msg-1",
        sessionId,
      },
      {
        ...
        message: { role: "system", content: "3s" },
        timestamp: new Date().toISOString(),
        parentUuid: "system-local-command",
        sessionId,
      },
      {
        type: "assistant",
        uuid: "msg-2",
        message: { role: "assistant", content: [{ type: "text", text: "end mixed range" }] },
        timestamp: new Date().toISOString(),
        parentUuid: "system-turn-duration",
        sessionId,
      },
    ]);
    const markerPath = getArchivedMarkerPath(sessionId);
    ...
    const response = await routeContextBonsaiTool(
      "context-bonsai-prune",
      {
        from_pattern: "start mixed range",
        to_pattern: "end mixed range",
        summary: "summary",
        index_terms: ["topic"],
      },
      {
    ...
    expect(response.content[0]?.text).toContain("Prune complete. anchor_id=msg-1");
    expect(response.isError).toBeUndefined();
    expect(await Bun.file(markerPath).json()).toEqual([
      "msg-1",
